...
class Kddcup:
    ...
    def get_clean_training_testing_data(self, ratio):
        ...
        self.data = np.zeros([self.num_train_points, 120], dtype=np.float64)
        self.test_data = np.zeros([self.num_test_points, 120], dtype=np.float64)
        self.test_labels = np.zeros([self.num_test_points, 1], dtype=np.float64)
        head = 0
        for i in range(self.num_assign_points):
            pl = float(self.xy[i][120])
            if pl < 1:
                for j in range(120):
                    self.data[head, j] = float(self.xy[i][j])
                head = head + 1
        for i in range(self.num_test_points):
            ni = self.num_assign_points + i
            for j in range(120):
                self.test_data[i, j] = float(self.xy[ni][j])
            self.test_labels[i, 0] = float(self.xy[ni][120])
        return self.data, self.test_data, self.test_labels, self.num_test_points
    ...
    def get_contaminated_training_testing_data(self, ratio, contamination):
        ...
        self.data = np.zeros([self.num_train_points, 120], dtype=np.float64)
        self.labels = np.zeros([self.num_train_points, 1], dtype=np.float64)
        self.test_data = np.zeros([self.num_test_points, 120], dtype=np.float64)
        self.test_labels = np.zeros([self.num_test_points, 1], dtype=np.float64)
        head = 0
        contaminated_cnt = 0
        ...
        for i in range(self.num_test_points):
            ...
            for j in range(120):
                self.test_data[i, j] = float(self.xy[ni][j])
            if self.xy[ni][120] < 1:
                self.test_labels[i, 0] = 0
            else:
                self.test_labels[i, 0] = 1
        return self.data, self.labels, self.test_data, self.test_labels, self.num_test_points
